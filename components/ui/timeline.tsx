...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className="w-full bg-none dark:bg-neutral-950 font-sans md:px-10"
      ref={containerRef}
    >
      <div className="max-w-7xl mx-auto text-center py-6">
        <h2 className="text-2xl md:text-4xl mb-3 font-extrabold text-white max-w-4xl mx-auto">
          Journey in Tech Industry
        </h2>
        <p className="text-white/90 text-sm md:text-base max-w-3xl mx-auto">
          Selected internships and hands-on engineering roles — focused on backend systems, API development, and full-stack product work.
        </p>
      </div>
      ...
      <div ref={ref} className="relative max-w-8xl mx-auto pb-20">
        {data.map((item, index) => (
          <div
            key={index}
            className="flex justify-start  md:pt-20 md:gap-8"
          >
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-bold text-white dark:text-white italic">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
